...
public class resolution {
	...
	public static void main(String[] args) {
		...
		GraphErrors ratio_points = new GraphErrors();
		for (int i=0; i<n_bins-1; i++) {
			...
			if (mc_values[i]!=0) {
				// println(bin_centers[i]+" "+rec_values[i]+" "+mc_values[i]);
				double error = Math.pow(rec_values[i]*(rec_values[i]+mc_values[i])/
					Math.pow(mc_values[i],3) ,0.5);
				ratio_points.addPoint(bin_centers[i], rec_values[i]/mc_values[i], 0, error);	
			}
		}

		ratio_points.setMarkerColor(1);
		ratio_points.setLineColor(1);
		ratio_points.setMarkerStyle(0);
		ratio_points.setMarkerSize(4);
		ratio_points.setLineThickness(2);
		...
		ratio_points.setTitleX("xB");
		ratio_points.setTitleY("Recon/MC Events");
		canvas.getPad(0).setTitle("Hadron Pair Fractional Virtual Photon Energy");
		canvas.setTitleSize(38);
		canvas.setAxisTitleSize(26);
		canvas.setAxisLabelSize(22);
		canvas.setStatBoxFontSize(18);
		canvas.draw(ratio_points);
