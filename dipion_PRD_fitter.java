...
public class dipion_PRD_fitter extends GenericKinematicFitter {

    protected final Double mybeam;

    public dipion_PRD_fitter(double beam) {
        super(beam);
        mybeam = beam;
    } 
        ...
        for (int current_Row = 0; current_Row < traj_Bank.rows(); current_Row++) {
            ...
            if (current_Part == traj_Bank.getInt("pindex", current_Row)) {
                ...
                if (region == 0 || region == 1 || region == 2) {
                    ...
                    int pid = rec_Bank.getInt("pid", current_Part);
                    switch (pid) { // these are to set the index in the parameter lists
                    case 11:
                        pid = 0;
                        break;
                    ...
                    if (run_Bank.getFloat("torus", 0) <0 && rec_Bank.getInt("pid", current_Part) != 11) {
                        // inbending hadrons
                        double calc_phi_min = minparams_in_had[pid][sector][region][0] + 
                            minparams_in_had[pid][sector][region][1]*Math.log(theta_DCr) + 
                            minparams_in_had[pid][sector][region][2]*theta_DCr +
                            minparams_in_had[pid][sector][region][3]*theta_DCr*theta_DCr;

                        double calc_phi_max = maxparams_in_had[pid][sector][region][0] + 
                            maxparams_in_had[pid][sector][region][1]*Math.log(theta_DCr) + 
                            maxparams_in_had[pid][sector][region][2]*theta_DCr +
                            maxparams_in_had[pid][sector][region][3]*theta_DCr*theta_DCr; 

                        track_success = phi_DCr > calc_phi_min && phi_DCr < calc_phi_max;
                    }  
                    else if (run_Bank.getFloat("torus", 0) <0 && rec_Bank.getInt("pid", current_Part) == 11) { 
                        // inbending electrons 
                        double x_New = 10000;
                        double y_New = 10000;
                        switch (sector) {
...
//                        System.out.println(rec_Bank.getInt("pid", current_Part));
                        double x_New = 10000;
                        double y_New = 10000;
                        switch (sector) {
                            case 0:
                        ...
                        double calc_min = minparams_out_elec_had[pid][sector][region][0]+minparams_out_elec_had[pid][sector][region][1]*x_New;
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public boolean electron_z_vertex_cut(float vz, HipoDataBank run_Bank) {
        if (run_Bank.getFloat("torus", 0)<=0) {
            return vz>-8 && vz<3;
        }
        if (run_Bank.getFloat("torus", 0)>0) {
            return vz>-10 && vz<2.5;
...
//            && electron_z_vertex_cut(vz, run_Bank)
